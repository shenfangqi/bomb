...
cc.log("nnnnn");
        ...
        //if in mobile, the viewport changed, it may couse joystick halt, to solve that ,do touchend process.
        if(target.getParent()._isTouchEnd == 1) {
            var target = event.getCurrentTarget();
            target.getParent()._target.endMove();
            target.getParent()._isTouchEnd = 0;
        ...
        target.getParent()._target.startMove();

        target.getParent()._isTouchEnd = 1;

        //如果触摸类型为FOLLOW，则摇控杆的位置为触摸位置,触摸开始时候现形
        if(target.getParent()._touchType == TouchType.FOLLOW)
        {
            touchOff = cc.p(touch.getLocation());
            if(touchOff.x > 600 || touchOff.y > 10*32) {
                return true;
            }
            //target.getParent().setPosition(touch.getLocation());
            target.getParent().setPosition(touchOff.x-bb.playRectOffX, touchOff.y-bb.playRectOffY);
            target.getParent().setVisible(true);
    ...
    onTouchMoved: function(touch, event)
    {
        ...
        target.getParent()._target.setTrace(touchPos.x, touchPos.y);
